...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # FIXED: Only compute FFT on unmasked patches to prevent spectral leakage
        if mask is not None:
            # Initialize spectral features as zeros
            spectral = torch.zeros(bz, ch_num, patch_num, 101, device=x.device)
            
            # Only compute FFT for unmasked patches
            unmasked_patches = (mask == 0)
            if torch.any(unmasked_patches):
                # Get unmasked data and compute FFT
                unmasked_data = mask_x.contiguous().view(bz, ch_num, patch_num, patch_size)[unmasked_patches]
                unmasked_spectral = torch.fft.rfft(unmasked_data, dim=-1, norm='forward')
                unmasked_spectral = torch.abs(unmasked_spectral)
                
                # Place unmasked spectral features back in their positions
                spectral[unmasked_patches] = unmasked_spectral
        else:
            # No masking - compute FFT for all patches
            original_x = x.contiguous().view(bz*ch_num*patch_num, patch_size)
            spectral = torch.fft.rfft(original_x, dim=-1, norm='forward')
