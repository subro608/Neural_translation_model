...
Other features:
  ...
  - fMRIDecodingAdapter is in TranslatorModel.__init__ (trainable + checkpointed)
...
from module import (  # type: ignore
    ...
    HierarchicalEncoder,
    CrossAttentionLayer,
    EEGDecodingAdapterLite,      # NEW
    fMRIDecodingAdapterLite,     # NEW
)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x: (B, T, D) -> returns (B, T, D) with positional encoding added
        """
...
class TranslatorModel(nn.Module):
    def __init__(
        ...
        # fMRI decoder (token sequence) + scalar head to reshape to (T,V)
...
class TrainConfig:
    ...
    # NEW: mixing controls
    mix_mode: str = "tri"  # {"tri","both","eeg_missing","fmri_missing","partial_eeg","partial_fmri"}
...
def _allocate_counts(total: int, k: int, weights: List[float], ensure_all: bool) -> List[int]:
    """
    Allocate 'total' items across k groups by weights.
    If ensure_all and total >= k, each group gets at least 1.
    """
    ...
    def run_epoch(mode: str, epoch: int):
        ...
        for batch in iterator:
            ...
            if cfg.mix_mode == "both":
                ...
                # groups: 0=both, 1=single-missing, 2=partial
                ...
                # split single-missing evenly (or as even as possible)
                if idx1.numel() > 0:
                    half = idx1.numel() // 2
                    miss_eeg[idx1[:half]] = True
                    miss_fmri[idx1[half:]] = True

                # split partial evenly (or as even as possible)
                if idx2.numel() > 0:
                    halfp = idx2.numel() // 2
            ...
            with torch.no_grad():
                ...
                # coords (A424 normalized if V==424, else zeros)
                try:
                    cand = [
...
def main() -> None:
    ...
    parser.add_argument('--mix_mode', type=str, default='tri',
                        choices=['tri','both','eeg_missing','fmri_missing','partial_eeg','partial_fmri'],
                        help='How to mix modalities within each batch.')
    parser.add_argument('--tri_ratios', type=float, nargs=3, default=[0.34, 0.33, 0.33],
                        metavar=('BOTH','SINGLE','PARTIAL'),
                        help='Ratios for tri mode (both, single-missing, partial).')
    parser.add_argument('--tri_ensure_all', action='store_true',
                        help='Ensure each tri factor appears at least once in a batch (if batch size allows).')
    parser.add_argument('--partial_visible_range', type=float, nargs=2, default=[0.30, 0.80],
                        metavar=('LO','HI'),
                        help='Contiguous visible fraction range for partial modes.')
...
# python train.py --mix_mode both
# python train.py --mix_mode fmri_missing
# python train.py --mix_mode eeg_missing
# python train.py --mix_mode partial_eeg --partial_visible_range 0.3 0.8
# python train.py --mix_mode partial_fmri --partial_visible_range 0.5 0.9
