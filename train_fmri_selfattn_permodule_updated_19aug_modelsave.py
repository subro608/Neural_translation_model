...
def make_dataloaders(cfg: TrainCfg, device: torch.device) -> Tuple[DataLoader, DataLoader, Optional[DataLoader]]:
    ...
    if cfg.test_subjects:
        if len(test_keys) == 0:
            raise RuntimeError("test_subjects were provided but produced no matching (subject, task, run) samples.")
        test_dl = _dl(test_keys, "test")
...
def _train_once_for_cfg(base_cfg: TrainCfg, stage: int, *, trial_name: str,
    ...
    # Warm-start (forward chain) — include affine
    if stage == 2:
        load_modules_if_exist(prev_stage_dir, translator, tag="stage1_best",
                            which=["adapter", "affine"], device=device)
    elif stage == 3:
        load_modules_if_exist(prev_stage_dir, translator, tag="stage2_best",
                            which=["adapter", "encoder", "affine"], device=device)
...
def main():
    ...
    if mode == "train" and cfg.auto_resume and _any_last_modules_exist(stage):
        ...
        if mode == "train":
            # Forward chain warm-starts carry affine too
            if stage == 2:
                load_modules_if_exist(out_dir, translator, tag="stage1_best",
                                    which=["adapter", "affine"], device=device)
            elif stage == 3:
                load_modules_if_exist(out_dir, translator, tag="stage2_best",
                                    which=["adapter", "encoder", "affine"], device=device)
        else:
