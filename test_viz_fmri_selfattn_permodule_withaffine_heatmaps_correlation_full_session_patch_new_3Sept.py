#!/usr/bin/env python3
"""
Diagnostics for fMRI-only (self-attn) per-module translator — now mirrors TRAINING exactly:
- Operates on 40s windows (or WINDOW_SEC) like training
- Resamples a patch-sparse mask **per window** (just like per-batch in training)
- Applies the mask **before** BrainLM
- Stitches the whole session with center-only keep
- Saves heatmaps and diagnostics; mask heatmap reflects the **kept regions only**
...
- PLOTS:
    ...
    * **Mask heatmap** showing where patch-sparse masking was applied (kept regions)
"""
...
# === PATCH-SPARSE MASKING (mirror trainer, per-window) =================
@torch.no_grad()
...
class FrozenBrainLM(nn.Module):
    def __init__(self, model_dir: Path, device: torch.device):
        ...
        def _infer_time_patch_size(model) -> int:
            ...
            cand_names = ["temporal_patch_size", "time_patch_size", "t_patch_size", "patch_size_time", "timepoint_patching_size"]
...
# =========================
# Main run (WINDOWED, per-window masking like training)
# =========================
...
def main():
    ...
    # =========================
    # Inference — WINDOWED like training (per-window mask resampling)
    # =========================
    chunk_T = max(1, int(WINDOW_SEC / float(TR)))          # e.g., 40s/2s = 20 TRs
    try:
        stride_T = max(1, int(STRIDE_SEC / float(TR)))     # e.g., 10s/2s = 5 TRs
    except NameError:
        stride_T = chunk_T
    stride_T = min(stride_T, chunk_T)

    print(f"[infer] chunked inference (center-only): WINDOW_SEC={WINDOW_SEC} → chunk_T={chunk_T} TRs, stride_T={stride_T} TRs, V={V}")

    recon_full = np.zeros((T, V), dtype=np.float32)
    # For visualization: store mask over the actually kept center regions
    mask_vis = np.zeros((T, V), dtype=np.bool_) if APPLY_TIME_MASK else None
    ...
    with torch.inference_mode():
        ...
        while start < T:
            ...
            # Resample a masking pattern PER WINDOW (like training)
            if APPLY_TIME_MASK:
                M_win = _make_patch_sparse_mask(
                    B=B, T=seg_T, V=V, device=fmri_seg.device,
                    patch_size=int(brainlm.time_patch_size),
                    r_patch_range=(0.5, 0.8),
                    r_v_in_patch_range=(0.5, 0.8),
                )  # True = masked
                fmri_seg = _apply_zero_mask(fmri_seg, M_win)
            else:
                M_win = None

            # Pad for BrainLM tokenization and get latents
            ...
            # Predict (B, seg_T, V)
            recon_chunk = translator(fmri_latents, fmri_T=seg_T, fmri_V=V, xyz=xyz)                         # (B,seg_T,V)
            r_np = recon_chunk[0].detach().cpu().numpy()                                                    # (seg_T, V)

            # Center-only keep (stitch)
            keep_len = min(stride_T, seg_T)
            center_off = max(0, (seg_T - keep_len) // 2)
            abs_keep_start = start + center_off
            abs_keep_end   = abs_keep_start + keep_len

            # Fill head for the very first window if needed
            if start == 0 and center_off > 0:
                recon_full[0:abs_keep_start, :] = r_np[0:center_off, :]

            # Keep center region
            recon_full[abs_keep_start:abs_keep_end, :] = r_np[center_off:center_off+keep_len, :]

            # Track the exact mask used over the kept region (for plotting)
            if APPLY_TIME_MASK and mask_vis is not None and M_win is not None:
                m_np = M_win[0].detach().cpu().numpy()  # (seg_T, V) True=masked
                mask_vis[abs_keep_start:abs_keep_end, :] = m_np[center_off:center_off+keep_len, :]

            # Fill tail for the last window if needed
            if end >= T:
                tail_off = center_off + keep_len
                if tail_off < seg_T:
                    recon_full[abs_keep_end:start+seg_T, :] = r_np[tail_off:seg_T, :]
    ...
    # Mask heatmap (binary over kept regions)
    if APPLY_TIME_MASK and mask_vis is not None:
        save_heatmap(
            mask_vis.astype(np.float32),
            out_dir / "plots" / "fmri_mask_heatmap.png",
            title="Applied patch-sparse mask (per-window; kept regions)",
            tr=float(TR),
            cmap="Reds",
            vlim=(0.0, 1.0),
        )
