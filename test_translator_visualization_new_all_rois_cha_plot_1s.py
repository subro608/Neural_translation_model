#!/usr/bin/env python3
"""
Top-k correlation visualization for EEG & fMRI (A424) + leakage checks.
Variant: EEG plots show ONLY 1 second (exactly 200 points) per channel.

- Correlations are computed on the full window (unchanged).
- EEG plot uses a 1-second slice starting at EEG_PLOT_START_SEC.
"""
...
OUT_DIR = r"D:\Neuroinformatics_research_2025\Multi_modal_NTM\viz_output_17augmri_test_fixed_patchfmri_1sEEG"
...
MODE = "eeg2fmri"  # keep both observed so GT is intact for correlation

# EEG plotting: show ONLY one second (200 points)
EEG_PLOT_START_SEC = 0   # change to choose the 1-second slice start (0..WINDOW_SEC-1)

# fMRI plotting downsample cap (unchanged)
...
def run_once(cfg: VizConfig, subset: str = "test", mode: str = "both"):
    ...
    if mode == "both":
        ...
        raise ValueError("Supported modes: 'both' | 'eeg2fmri' | 'fmri2eeg'")
    ...
    # Compute correlations on first item of batch (full-window)
    b = 0
    ...
    # ---------- Plotting: Top-10 EEG (ONLY 1 second, 200 points) ----------
    if len(top_eeg) > 0:
        ...
        # Determine slice for exactly 1 second (S_ samples)
        sec = int(max(0, min(EEG_PLOT_START_SEC, WINDOW_SEC - 1)))
        start = sec * S_
        end   = start + S_  # exclusive
        # Time axis: seconds (absolute position in window)
        t_eeg_1s = (np.arange(end - start) / float(TARGET_FS)) + sec

        for ax, (ch, r) in zip(axes_eeg, top_eeg):
            gt_flat = x_true_eeg[ch].reshape(Pe * S_)
            rc_flat = x_rec_eeg[ch].reshape(Pe * S_)
            # Guard slice in bounds
            end_clamped = min(end, gt_flat.shape[0])
            start_clamped = min(start, end_clamped - 1)
            gt_1s = gt_flat[start_clamped:end_clamped]
            rc_1s = rc_flat[start_clamped:end_clamped]
            # If due to bounds we got fewer than S_ points (shouldn't happen with sane params), pad time
            t_plot = t_eeg_1s[:len(gt_1s)]

            ax.plot(t_plot, gt_1s, label="EEG GT")
            ax.plot(t_plot, rc_1s, label="EEG Recon", alpha=0.85)
            ax.set_title(f"EEG Channel {ch} — r(full)={r:.3f}  •  slice=[{sec}s, {sec+1}s)")
            ax.set_xlabel("Time (s)"); ax.set_ylabel("Amplitude (z-sc)")
            ax.legend(loc="upper right")

        fig_eeg.tight_layout()
        fig_eeg.savefig(out_dir / "plots" / f"top10_eeg_corr_1s_start{EEG_PLOT_START_SEC}.png", dpi=150)
        plt.close(fig_eeg)

    # ---------- Plotting: Top-10 fMRI (unchanged, full T) ----------
    def downsample_fmri(x: np.ndarray, y: np.ndarray, max_points: int) -> Tuple[np.ndarray, np.ndarray]:
        n = x.shape[0]
        if n <= max_points:
            return x, y
        step = int(math.ceil(n / max_points))
    ...
    if len(top_fmri) > 0:
        ...
        if len(top_fmri) == 1:
            axes_fmri = [axes_fmri]
        t_fmri = np.arange(T_) * float(TR)
        for ax, (roi, r) in zip(axes_fmri, top_fmri):
            gt = x_true_fmri[:, roi]
            rc = x_rec_fmri[:, roi]
            tf, gt_d = downsample_fmri(t_fmri, gt, PLOT_MAX_POINTS_FMRI)
            _,  rc_d = downsample_fmri(t_fmri, rc, PLOT_MAX_POINTS_FMRI)
    ...
    # Console summary
    print("\n=== Top EEG channels by Pearson r (full-window) ===")
    for ch, r in top_eeg:
        print(f"  ch={ch:02d}  r={r:.4f}")
    print(f"\n[EEG plot] 1-second slice starting at t={EEG_PLOT_START_SEC}s → exactly {TARGET_FS} points.")
