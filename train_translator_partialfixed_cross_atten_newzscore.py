#!/usr/bin/env python3
# train_translator_fixed_fmri2eeg.py

"""
Train the adapter_main_translatordecoder on paired, aligned Oddball EEG+fMRI windows,
Stage 2: adapters + Lite decoders + affine + cross-attention trainable.
Encoders remain frozen. Norm γ/β and all biases use no weight decay.
Cross-attn LR = 0.25× base. Dropout=0.05. Variance floor on pre-tanh fMRI.
"""
...
from module_translator_cross import (  # type: ignore
    ...
    TranslatorModel,              # now exports forward(..., return_pretanh)
)
...
from data_oddball_zscore_sess import (  # type: ignore
...
# -----------------------------
# Positional Encoding (sin/cos) — used by module
# -----------------------------
...
# -----------------------------
# Freeze helpers (Stage 2)
# -----------------------------
def set_stage2_requires_grad(translator: TranslatorModel) -> None:
    """Stage 2: adapters + Lite decoders + affine + cross-attn trainable; encoders frozen."""
    for p in translator.parameters():
        p.requires_grad = False
    # Stage 1 trainables
    for m in [translator.adapter_eeg, translator.adapter_fmri, translator.eeg_decoder, translator.fmri_decoder]:
        for p in m.parameters(): p.requires_grad = True
    translator.fmri_out_scale.requires_grad = True
    translator.fmri_out_bias.requires_grad  = True
    # Stage 2 addition
    for p in translator.cross_attn.parameters(): p.requires_grad = True
    # (compressor has no params; encoders remain frozen)
...
class TrainConfig:
    ...
    # mixing controls
    mix_mode: str = "tri"
    tri_ratios: List[float] = field(default_factory=lambda: [0.34, 0.33, 0.33])
    ...
    # variance floor
    var_floor_target: float = 0.15
    var_floor_lambda: float = 1e-2

# -----------------------------
# WD helpers (no-WD on norms & all biases)
# -----------------------------
def _norm_param_names(module: nn.Module) -> set[str]:
    norm_classes = (nn.LayerNorm, nn.GroupNorm, nn.BatchNorm1d, nn.InstanceNorm1d, nn.SyncBatchNorm)
    names = set()
    for mod_name, m in module.named_modules():
        if isinstance(m, norm_classes):
            for pn, _ in m.named_parameters(recurse=False):  # 'weight' (gamma), 'bias' (beta)
                names.add(f"{mod_name}.{pn}" if mod_name else pn)
    return names

def _build_stage2_param_groups(tr: TranslatorModel, base_lr: float, base_wd: float):
    norm_names = _norm_param_names(tr)
    cross_prefix = "cross_attn."
    groups = {
        "other_decay":    {"params": [], "lr": base_lr,        "weight_decay": base_wd},
        "other_nodc":     {"params": [], "lr": base_lr,        "weight_decay": 0.0},
        "cross_decay":    {"params": [], "lr": base_lr * 0.25, "weight_decay": base_wd},
        "cross_nodc":     {"params": [], "lr": base_lr * 0.25, "weight_decay": 0.0},
    }
    total = 0
    for n, p in tr.named_parameters():
        if not p.requires_grad:
            continue
        total += p.numel()
        is_cross = n.startswith(cross_prefix)
        is_nodc = (n in norm_names) or n.endswith(".bias")
        key = ("cross_" if is_cross else "other_") + ("nodc" if is_nodc else "decay")
        groups[key]["params"].append(p)
    pglist = [groups["other_decay"], groups["other_nodc"], groups["cross_decay"], groups["cross_nodc"]]
    # prune empties
    pglist = [g for g in pglist if len(g["params"]) > 0]
    print(f"[wd] stage2 param groups -> total trainable params: {total:,}")
    for i,g in enumerate(pglist):
        cnt = sum(p.numel() for p in g["params"])
        print(f"  group {i}: lr={g['lr']:.2e} wd={g['weight_decay']:.1e} | {cnt:,} params")
    return pglist
...
# -----------------------------
# Mask builders / EEG helpers (unchanged)
# -----------------------------
...
def train_loop(cfg: TrainConfig) -> None:
    ...
    if cfg.strict_subjects:
        ...
        train_keys, val_keys, test_keys = fixed_subject_keys(
            cfg.eeg_root, cfg.fmri_root, cfg.train_subjects, cfg.val_subjects, cfg.test_subjects
        )
    else:
    ...
    # DataLoaders (deterministic, no shuffle)
    ...
    # Translator (dropout=0.05 per Stage 1/2 guidance)
    ...
    translator = TranslatorModel(
        ...
        d_model=128, n_heads=4, d_ff=512, dropout=0.05,
        voxel_count=fmri_target_V,
        debug=cfg.debug,
    ).to(device)

    # Stage 2 requires_grad
    set_stage2_requires_grad(translator)

    # Optimizer & scheduler (Stage 2 groups)
    param_groups = _build_stage2_param_groups(translator, cfg.lr, cfg.weight_decay)
    optim = torch.optim.AdamW(param_groups, lr=cfg.lr, weight_decay=0.0)
    ...
    if not cfg.wandb_off:
        run = wandb.init(
            ...
            name=cfg.wandb_run_name or "translator_stage2",
            config={**asdict(cfg), "stage": "2", "model": "Translator(Stage2)+CBraMod(frozen)+BrainLM(frozen)"},
        )
        try:
    ...
    trainable_names = [n for n,p in translator.named_parameters() if p.requires_grad]
    ...
    def run_epoch(mode: str, epoch: int):
        ...
        for batch in iterator:
            ...
            # For mix_mode behavior reuse original logic (unchanged); here we run 'tri' by default
            # --- allocate masks ---
            ...
            with torch.amp.autocast('cuda', enabled=(cfg.amp and device.type == 'cuda')):
                ...
                recon_eeg, recon_fmri, pre_tanh = out
                ...
                # variance floor on pre-tanh fMRI
                std = pre_tanh.flatten(1).std(dim=1)
                var_penalty = torch.relu(cfg.var_floor_target - std).mean() * cfg.var_floor_lambda
                ...
                loss = cfg.eeg_loss_w * loss_eeg + cfg.fmri_loss_w * loss_fmri + var_penalty
            ...
            if is_train:
                ...
                # clamp affine
                with torch.no_grad():
                    translator.fmri_out_scale.clamp_(0.25, 4.0)
                    translator.fmri_out_bias.clamp_(-2.0, 2.0)
                global_step += 1

            total_eeg_loss += float(loss_eeg.detach().cpu())
            total_fmri_loss += float(loss_fmri.detach().cpu())
            total_var_pen += float(var_penalty.detach().cpu())
            steps += 1

            if tqdm is not None:
                iterator.set_postfix_str(
                    f"loss:{float(loss.detach().cpu()):.3f} std:{float(std.mean().detach().cpu()):.3f} "
                    f"lr:{optim.param_groups[0].get('lr', cfg.lr):.2e}"
                )
            ...
            if is_train and not cfg.wandb_off:
                wandb.log({
                    ...
                    "train/step_var_penalty": float(var_penalty.detach().cpu()),
                    "train/pretanh_std_mean": float(std.mean().detach().cpu()),
                    "train/lr_group0": float(optim.param_groups[0].get("lr", 0.0)),
                    "train/lr_group1": float(optim.param_groups[1].get("lr", 0.0)) if len(optim.param_groups)>1 else 0.0,
                }, step=global_step)

        return {
            f'{mode}_eeg_loss': total_eeg_loss / max(1, steps),
            f'{mode}_fmri_loss': total_fmri_loss / max(1, steps),
            f'{mode}_var_penalty': total_var_pen / max(1, steps),
        }
    ...
    for epoch in range(start_epoch, cfg.num_epochs + 1):
        ...
        print(f"Epoch {epoch:02d} | {dur:.1f}s | "
              f"train EEG {tr['train_eeg_loss']:.6f} FMRI {tr['train_fmri_loss']:.6f} (var {tr['train_var_penalty']:.5f}) | "
              f"val EEG {va['val_eeg_loss']:.6f} FMRI {va['val_fmri_loss']:.6f} (var {va['val_var_penalty']:.5f})")

        # Save "last"
        last_path = Path(cfg.output_dir) / 'translator_last.pt'
        best_path = Path(cfg.output_dir) / 'translator_best.pt'
        val_score = va['val_eeg_loss'] + va['val_fmri_loss'] + va['val_var_penalty'] * 0.0  # don't rank by var penalty
...
def dry_run_debug(
    ...
    with torch.no_grad():
        out = translator(eeg_latents, fmri_latents, T, V, return_pretanh=True)
        eeg_signal, fmri_signal, pre_tanh = out
        print("[eeg_signal] ->", shp(eeg_signal), "(expect (B,C,Pe,S) =", (batch_size, eeg_channels, Pe, S), ")")
        print("[fmri_signal] ->", shp(fmri_signal), "(expect (B,T,V) =", (batch_size, T, V), ")")
        print("[pre_tanh std] ->", float(pre_tanh.flatten(1).std(dim=1).mean().cpu()))
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Stage-2 training: adapters+decoders+affine+cross-attn (encoders frozen)')
    ...
    parser.add_argument('--output_dir', type=str, default=str(THIS_DIR / 'translator_runs_stage2'))
    ...
    parser.add_argument('--tri_ratios', type=float, nargs=3, default=[0.34, 0.33, 0.33])
    ...
    parser.add_argument('--partial_visible_range', type=float, nargs=2, default=[0.30, 0.80])
    # variance floor
    parser.add_argument('--var_floor_target', type=float, default=0.15)
    parser.add_argument('--var_floor_lambda', type=float, default=1e-2)
    ...
    if temp_args.config is not None:
        ...
        file_cfg = file_all.get('train', file_all) if isinstance(file_all, dict) else {}
    ...
    cfg = TrainConfig(
        ...
        var_floor_target=float(args.var_floor_target),
        var_floor_lambda=float(args.var_floor_lambda),
    )
...
# Examples:
# python train_translator_fixed_fmri2eeg.py --config configs/translator.yaml --epochs 20 --device cuda --wandb_project yourproj --wandb_run_name stage2
# python train_translator_fixed_fmri2eeg.py --dry_run
