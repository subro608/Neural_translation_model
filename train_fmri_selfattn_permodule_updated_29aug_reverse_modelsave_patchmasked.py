...
with **patch-sparse** masking applied BEFORE BrainLM, and mandatory voxel positional
encoding (learned ID + XYZ MLP) added at the token-grid level.

Masking rules (always on, no flags):
- Let patch_size = 20 TRs (BrainLM temporal tokenization).
- Sample r_patch ~ Uniform[0.3, 0.8], pick that fraction of **time patches**.
- For each selected patch, sample r_v_patch ~ Uniform[0.3, 0.8] and mask
  that **fraction of ROIs only within that patch** (NOT all ROIs).
...
# === PATCH-SPARSE MASKING ==============================================
@torch.no_grad()
def _make_patch_sparse_mask(
    B: int, T: int, V: int, device: torch.device,
    *,
    patch_size: int = 20,
    r_patch_range = (0.5, 0.8),        # fraction of patches to select
    r_v_in_patch_range = (0.5, 0.8),   # fraction of ROIs masked within each selected patch
) -> torch.Tensor:
    """
    Returns boolean mask M (B, T, V) with True = masked.
    - Split T into patches of length patch_size (last patch may be shorter).
    - Sample a fraction of patches r_patch in [0.3, 0.8].
    - For EACH selected patch, sample r_v_patch in [0.3, 0.8] and mask that
      fraction of ROIs, across the full temporal span of the patch.
    - Same mask is applied to all items in the batch (simple & fast).
    """
    # number of patches (ceil for final partial patch)
...
def run_epoch(mode:str, dl, model, brainlm, xyz_ref, cfg: TrainCfg, device, scaler, opt=None):
    ...
    for bidx, batch in enumerate(iterator):
        ...
        # === PATCH-SPARSE MASKING — build mask & zero masked entries BEFORE BrainLM ===
        M = _make_patch_sparse_mask(B, T, V, device=fmri_t.device, patch_size=20)  # True = masked
...
# -----------------------------
def main():
    ap = argparse.ArgumentParser(description="Reversed (decoder-first) stage-wise fMRI-only training/testing (axial RoPE + 2-D decoder) with PATCH-sparse pre-BrainLM masking + voxel positional encoding + per-module checkpoints + W&B + Auto-Resume + Auto-Sweep + Diagnostics")
