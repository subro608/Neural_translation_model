#!/usr/bin/env python3
"""
Visualization for fMRI-only (self-attn) per-module translator — static config (no CLI args)

- Restores modules from a training run directory (adapter/encoder/decoder/affine).
- Loads one batch from the chosen split (train/val/test) using PairedAlignedDataset.
- Frozen BrainLM -> BrainLM latents -> TranslatorFMRISelfAttn -> fMRI recon.
- Computes Pearson r per ROI; saves:
    * plots/topK_fmri_corr.png   (Top-K ROI GT vs Recon time-series)
    * plots/corr_bars_fmri.png   (bar chart of all ROIs, sorted by r)
    * tables/fmri_roi_pearson_r.csv
- Reads subject_splits.json from run_dir when present; otherwise makes deterministic split.
"""
...
from pathlib import Path  # (already imported lower; keep it here too)
...
RUN_DIR           = Path(r"D:\Neuroinformatics_research_2025\Multi_modal_NTM\runs_fmri_selfattn_18aug")
OUT_DIR           = Path(r"D:\Neuroinformatics_research_2025\Multi_modal_NTM\viz_fmri_selfattn_permodule")


SUBSET              = "test"   # "train" | "val" | "test"
STAGE               = 1         # 1 | 2 | 3  (controls which modules must be present)
TAG                 = None       # e.g., "stage2_last"; None -> uses f"stage{STAGE}_best"
TOP_K               = 12
...
# Robust BrainLM import pathing (works if BrainLM lives in repo or elsewhere)
candidate_blm_roots = [
    BRAINLM_DIR,
    THIS_DIR / "BrainLM",
...
def load_modules_if_exist(run_dir: Path, model: TranslatorFMRISelfAttn, tag: str,
                          which: Iterable[str], device: torch.device):
...
def main():
    ...
    # XYZ coordinates (A424). If V!=424, fallback to zeros.
    try:
        candidates = [
