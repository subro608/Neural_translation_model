...
with **mix_mode active** and **only adapters + Lite decoders trainable**.

Key behavior:
  - Freeze EVERY translator parameter except:
      * adapter_eeg (ConvEEGInputAdapter)
      * adapter_fmri (fMRIInputAdapterConv1d)
      * eeg_decoder (EEGDecodingAdapterLite)
      * fmri_decoder (fMRIDecodingAdapterLite)
    (fmri_out_scale/bias stay frozen; flip their requires_grad if you want them trainable.)
  - Use the original `mix_mode` logic to mix both/partial/missing cases within each batch.
  - CBraMod & BrainLM are frozen feature extractors.
...
Other features:
  - STRICT SUBJECT SPLITS FROM YAML when --config has subject IDs
  - Deterministic splits, no DataLoader shuffle
  ...
  - Sinusoidal positional encodings added to both token sequences (buffers)
  - Translator parameter budget reduced (~1.4M in the base design)
"""
...
# -----------------------------
# Translator (adapters + decoders will be trainable; rest frozen by helper)
# -----------------------------
class TranslatorModel(nn.Module):
    def __init__(
        ...
        # Adapters (these will remain trainable)
        ...
        # Lite decoders (these will remain trainable)
        ...
        # Output affine for fMRI (kept frozen unless you opt-in to unfreeze)
...
# -----------------------------
# Freeze helper — leave ONLY adapters + Lite decoders trainable
# -----------------------------
def freeze_translator_except_adapters_and_decoders(translator: TranslatorModel) -> None:
    for name, param in translator.named_parameters():
        if (
            name.startswith("adapter_eeg.")
            or name.startswith("adapter_fmri.")
            or name.startswith("eeg_decoder")
            or name.startswith("fmri_decoder")
            or name.startswith("fmri_out_scale")
            or name.startswith("fmri_out_bias")
        ):
            param.requires_grad = True
        else:
            param.requires_grad = False
    # If you also want these trainable, uncomment below:
...
# -----------------------------
# Training loop — mix_mode active; adapters+decoders only trainable
# -----------------------------
def train_loop(cfg: TrainConfig) -> None:
    ...
    # Freeze everything except adapters + decoders
    freeze_translator_except_adapters_and_decoders(translator)

    # Optimizer & scheduler (only params with requires_grad=True)
    ...
    if not cfg.wandb_off:
        run = wandb.init(
            ...
            config={**asdict(cfg), "model": "Translator(adapters+decoders-trainable)+CBraMod(frozen)+BrainLM(frozen)"},
        )
        try:
...
# -----------------------------
# Dry-run utility (reflects freeze helper)
# -----------------------------
def dry_run_debug(
    device: torch.device,
    window_sec: int = 30,
...
# -----------------------------
def main() -> None:
    parser = argparse.ArgumentParser(description='Train translator-decoder on paired Oddball EEG+fMRI with mix_mode and adapters+decoders-only training')
...
# Examples:
# python train_translator_paired_mixmode_adapters_decoders_trainable.py --config configs/translator.yaml
# python train_translator_paired_mixmode_adapters_decoders_trainable.py --config configs/translator.yaml --mix_mode penta --penta_ensure_all
# python train_translator_paired_mixmode_adapters_decoders_trainable.py --config configs/translator.yaml --dry_run
