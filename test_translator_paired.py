#!/usr/bin/env python3
"""
Test / Inference for the tri-mix EEG↔fMRI translator (parity with training).

- Uses the exact TranslatorModel path from training:
  fMRIDecodingAdapter + token-scalar head + tanh + learnable affine.
- Builds translator with fmri_tokens_target = T * V (NOT BrainLM Ttok).
- Deterministic subject split: prefers subject_splits.json from train.output_dir,
  otherwise uses test.{train,val,test}_subjects from config.
- Metrics:
  * both/eeg2fmri/fmri2eeg: full MSE + Pearson corr
  * partial_eeg/partial_fmri: masked-only MSE + Pearson corr (only on hidden time)
"""
...
def batch_corr(a: torch.Tensor, b: torch.Tensor, dims) -> torch.Tensor:
    """
    Pearson correlation per sample across `dims`. Returns (B,) tensor (or scalar if B collapsed).
    """
...
def _rand_block_mask_time(B: int, L: int, vis_frac: float, device) -> torch.Tensor:
    """
    Return (B,L) where True==MASKED. One visible contiguous block of length vis_frac*L.
    """
...
def _group_mask_seconds(M_time: torch.Tensor, seconds_per_token: int) -> torch.Tensor:
    """
    Group a per-second time mask (B,P) into (B,Pe) with "any" policy over each group.
    """
    B, P = M_time.shape
...
class FrozenBrainLM(nn.Module):
    ...
    def extract_latents(self, signal_vectors: torch.Tensor, xyz_vectors: torch.Tensor) -> torch.Tensor:
        """
        signal_vectors: (B, V, Tp)  ; xyz_vectors: (B, V, 3)
        returns: (L, B, Ttok, H)
        """
...
# --------------- Load subject_splits.json if available ---------------
def _load_fixed_subjects_from_json(train_output_dir: Optional[Path]) -> Optional[Tuple[List[int], List[int], List[int]]]:
...
def run_test(cfg: TestConfig):
    ...
    # Deterministic subject-run split
    inter_keys = collect_common_sr_keys(cfg.eeg_root, cfg.fmri_root)
    if len(inter_keys) == 0:
        print("No paired aligned (subject,run) found. Check paths.")
        return
    ...
    ds = PairedAlignedDataset(
        ...
        window_sec=cfg.window_sec, original_fs=cfg.original_fs, target_fs=cfg.target_fs,
        tr=2.0, channels_limit=cfg.channels_limit, fmri_norm=cfg.fmri_norm,
        stride_sec=cfg.stride_sec, device='cpu', include_sr_keys=test_keys,
    )
    if len(ds) == 0:
    ...
    dl = DataLoader(
        ...
        collate_fn=collate_paired, pin_memory=(device.type == 'cuda')
    )

    # Frozen feature extractors
    frozen_eeg  = FrozenCBraMod(cfg.target_fs, cfg.target_fs, cfg.window_sec, 12, 8, 800, cfg.cbramod_weights, device)
    ...
    fmri_hidden_dim = int(getattr(frozen_fmri.model.config, "hidden_size", 256))

    # accumulators
    mse_eeg_sum = mse_fmri_sum = 0.0
    corr_eeg_sum = corr_fmri_sum = 0.0
    n_eeg = n_fmri = 0
    ...
    translator: Optional[TranslatorModel] = None
    ...
    with torch.no_grad():
        for b_idx, batch in enumerate(iterator):
            ...
            # Lazily build translator with fmri_tokens_target = T * V (parity)
            if translator is None:
                ...
                # Load translator weights (filter shape-mismatched keys)
                try:
            ...
            if cfg.mode == "both":
                ...
                M_t = _rand_block_mask_time(B, T, cfg.partial_visible_frac, device)
            ...
            # Coordinates (A424 normalized if V==424, else zeros)
            try:
                cand = [
            ...
            if cfg.mode in ("both", "fmri2eeg", "partial_eeg"):
                if cfg.mode == "partial_eeg" and M_eeg_time is not None:
                    M_grp = _group_mask_seconds(M_eeg_time, cfg.eeg_seconds_per_token)  # (B,Pe)
                    diff = (recon_eeg - x_eeg_grp) ** 2          # (B,C,Pe,S)
                    diff_cs = diff.mean(dim=(1, 3))              # (B,Pe)
                    masked_mse = (diff_cs * M_grp.float()).sum(dim=1) / M_grp.sum(dim=1).clamp_min(1)
                    mse_eeg = masked_mse

                    # masked-only corr: flatten only masked tokens
                    corr_vals = []
                    for b in range(B):
                        idx = M_grp[b].nonzero(as_tuple=False).flatten()
                        if idx.numel() == 0:
                            continue
                        a = recon_eeg[b, :, idx, :].reshape(-1)
                        d = x_eeg_grp[b, :, idx, :].reshape(-1)
                        corr_vals.append(batch_corr(a[None, ...], d[None, ...], dims=(1,)).item())
            ...
            if cfg.mode in ("both", "eeg2fmri", "partial_fmri"):
                if cfg.mode == "partial_fmri" and M_t is not None:
                    diff = (recon_fmri - fmri_t) ** 2  # (B,T,V)
                    mask = M_t.unsqueeze(-1)           # (B,T,1)
                    masked_mse = (diff * mask).sum(dim=(1, 2)) / mask.sum(dim=(1, 2)).clamp_min(1)
                    mse_fmri = masked_mse

                    corr_vals = []
                    for b in range(B):
                        idx = M_t[b].nonzero(as_tuple=False).flatten()
                        if idx.numel() == 0:
                            continue
                        a = recon_fmri[b, idx, :].reshape(-1)
                        d = fmri_t[b,    idx, :].reshape(-1)
                        corr_vals.append(batch_corr(a[None, ...], d[None, ...], dims=(1,)).item())
            ...
            # Optional: save a few recon batches
            if saver_enabled and saved_batches < cfg.save_n_batches:
                out_dir = cfg.save_recons / f"batch_{b_idx:04d}"
                ensure_dir(out_dir)
                torch.save({
                    "recon_eeg": recon_eeg.cpu(),
                    "target_eeg_grouped": x_eeg_grp.cpu(),
                    "recon_fmri": recon_fmri.cpu(),
                    "target_fmri": fmri_t.cpu(),
                    "mode": cfg.mode,
                }, out_dir / "recons.pt")
                saved_batches += 1
...
# ----------------------------- CLI -----------------------------
def main():
    ap = argparse.ArgumentParser(description="Test EEG↔fMRI translator (parity with training; uses translator.yaml).")
    ap.add_argument('--config', type=str, required=True, help='Path to translator.yaml (or JSON). Uses the "test" section for defaults.')
    ...
    # Parse once to read config path
    ...
    # Pull defaults from "test" section; also peek at "train" for output_dir (subject_splits.json)
    test_cfg = (file_cfg.get('test') if isinstance(file_cfg, dict) else None) or {}
    train_cfg = (file_cfg.get('train') if isinstance(file_cfg, dict) else None) or {}

    # CLI overrides on top of test section
    def _get(k, default=None):
        v = test_cfg.get(k, default)
        return v

    cfg = TestConfig(
        eeg_root=Path(_get('eeg_root')),
        fmri_root=Path(_get('fmri_root')),
        a424_label_nii=Path(_get('a424_label_nii')),
        cbramod_weights=Path(_get('cbramod_weights')),
        brainlm_model_dir=Path(_get('brainlm_model_dir')),
        checkpoint=Path(_get('checkpoint')),
        device=(args.device or _get('device', 'cuda')),
        seed=int(_get('seed', 42)),
        fmri_norm=str(_get('fmri_norm', 'zscore')),
        window_sec=int(_get('window_sec', 40)),
        original_fs=int(_get('original_fs', 1000)),
        target_fs=int(_get('target_fs', 200)),
        stride_sec=(int(_get('stride_sec')) if _get('stride_sec') is not None else None),
        channels_limit=int(_get('channels_limit', 34)),
        batch_size=int(_get('batch_size', 8)),
        num_workers=int(_get('num_workers', 0)),
        eeg_seconds_per_token=int(_get('eeg_seconds_per_token', 40)),
        mode=(args.mode or _get('mode', 'eeg2fmri')),
        partial_visible_frac=(args.partial_visible_frac if args.partial_visible_frac is not None else float(_get('partial_visible_frac', 0.5))),
        save_recons=(Path(args.save_recons) if args.save_recons else (Path(_get('save_recons')) if _get('save_recons') else None)),
        save_n_batches=(int(args.save_n_batches) if args.save_n_batches is not None else int(_get('save_n_batches', 0))),
        train_subjects=_get('train_subjects'),
        val_subjects=_get('val_subjects'),
        test_subjects=_get('test_subjects'),
        train_output_dir=(Path(train_cfg.get('output_dir')) if train_cfg and train_cfg.get('output_dir') else None),
    )
