...
def _train_once_for_cfg(base_cfg: TrainCfg, stage: int, *, trial_name: str,
    ...
    for ep in range(1, epochs+1):
        ...
        if va["total"] < best:
            best = va["total"]
            save_modules(cfg.out_dir, translator, tag=f"revstage{stage}_best")
            save_trainstate(cfg.out_dir, f"revstage{stage}", tag="best", epoch=ep, best_val=best, opt=opt, scaler=scaler)
...
# -----------------------------
def main():
    ap = argparse.ArgumentParser(description="Reversed (decoder-first) stage-wise fMRI-only training/testing (self-attn) + per-module checkpoints + W&B + Auto-Resume + Auto-Sweep")
