...
- DIAGNOSTICS CSV (per-ROI):
    roi, r0, std_gt, std_rec, slope, intercept, R2, NRMSE, best_lag_TR, r_at_best_lag,
    slope_tv, intercept_tv, R2_tv, NRMSE_tv
  where:
    * slope/intercept/R2/NRMSE are FIT ON THE CURRENT BATCH (for quick visual debugging).
    * *_tv metrics are computed using train+val calibration applied to this batch.
- TRAIN/VAL CALIBRATION:
    * Fits one (a,b) per ROI across the specified splits (default ["train","val"])
    * Saves to tables/calibration_ab_trainval.csv
    * Used for calibrated test plots.
- PLOTS: top-K GT vs Recon (raw) + optional top-K GT vs Recon (train/val-calibrated)
...
Outputs:
  ...
  tables/calibration_ab_trainval.csv    (when CALIB_FIT_SPLITS != [])
"""
...
# NEW: train/val calibration control
CALIB_FIT_SPLITS   = ["val"]  # [] -> disable and revert to per-batch fit for plots
CALIB_SAVE_CSV     = OUT_DIR / "tables" / "calibration_ab_trainval.csv"
...
# =========================
# Train/Val calibration fitter
# =========================
def fit_calibration_over_splits(
    splits: List[str],
    eeg_root: Path, fmri_root: Path, run_dir: Path,
    brainlm: FrozenBrainLM, translator: TranslatorFMRISelfAttn,
    window_sec: int, tr: float, channels_limit: int, fmri_norm: str, stride_sec: int,
    coords_t: Optional[torch.Tensor],
    device: torch.device
) -> Tuple[np.ndarray, np.ndarray]:
    """Return per-ROI (a,b) fit across the specified splits, streaming in time."""
    train_keys, val_keys, test_keys = load_or_make_split(Path(eeg_root), Path(fmri_root), Path(run_dir))
    split_map = {"train": train_keys, "val": val_keys, "test": test_keys}

    N = None; sum_x = None; sum_y = None; sum_x2 = None; sum_xy = None
    for split in splits:
        include_sr = split_map[split]
        ...
        for batch in dl:
            fmri_t = batch['fmri_window'].to(device)  # (1,T,V)
            B,T,V = map(int, fmri_t.shape)

            fmri_pad = pad_timepoints_for_brainlm_torch(fmri_t, patch_size=20)  # (1,Tp,V)
            signal_vectors = fmri_pad.permute(0,2,1).contiguous()               # (1,V,Tp)

            if coords_t is not None and V == coords_t.shape[0]:
                xyz = coords_t.unsqueeze(0).repeat(B,1,1)
            else:
                xyz = torch.zeros(B, V, 3, device=device)

            with torch.no_grad():
                lat = brainlm.extract_latents(signal_vectors, xyz)  # (L,1,Ttok,Dh)
                rc  = translator(lat, fmri_T=T, fmri_V=V)           # (1,T,V)

            gt_np = fmri_t[0].detach().cpu().numpy()  # (T,V)
            rc_np = rc[0].detach().cpu().numpy()      # (T,V)

            if N is None:
                N      = np.zeros(V, dtype=np.int64)
                sum_x  = np.zeros(V, dtype=np.float64)
                sum_y  = np.zeros(V, dtype=np.float64)
                sum_x2 = np.zeros(V, dtype=np.float64)
                sum_xy = np.zeros(V, dtype=np.float64)

            N      += gt_np.shape[0]
            sum_x  += rc_np.sum(axis=0)
            sum_y  += gt_np.sum(axis=0)
            sum_x2 += (rc_np**2).sum(axis=0)
            sum_xy += (rc_np*gt_np).sum(axis=0)

    eps = 1e-8
    mean_x = sum_x / (N + eps)
    mean_y = sum_y / (N + eps)
    var_x  = sum_x2 / (N + eps) - mean_x**2
    cov_xy = sum_xy / (N + eps) - mean_x*mean_y
...
def main():
    ...
    # Subject split & dataset (for the chosen SUBSET batch)
    train_keys, val_keys, test_keys = load_or_make_split(Path(EEG_ROOT), Path(FMRI_ROOT), Path(RUN_DIR))
    ...
    # ===== Train/Val calibration (fits a,b per ROI once) =====
    a_vec = b_vec = None
    if CALIB_FIT_SPLITS:
        a_np, b_np = fit_calibration_over_splits(
            CALIB_FIT_SPLITS, EEG_ROOT, FMRI_ROOT, RUN_DIR,
            brainlm, translator, WINDOW_SEC, TR, CHANNELS_LIMIT, FMRI_NORM, STRIDE_SEC,
            coords if (coords is not None and coords.ndim == 2 and coords.shape[0] == V) else None,
            device
        )
        os.makedirs(out_dir / "tables", exist_ok=True)
        with open(CALIB_SAVE_CSV, "w", newline="") as f:
            w = csv.writer(f); w.writerow(["roi","slope","intercept"])
            for roi,(aa,bb) in enumerate(zip(a_np, b_np)):
                w.writerow([roi, float(aa), float(bb)])
        print(f"[calib] saved train+val calibration -> {CALIB_SAVE_CSV}")
        a_vec = torch.tensor(a_np, device=device)
        b_vec = torch.tensor(b_np, device=device)

    # ===== Reconstruct fMRI for the selected SUBSET batch =====
    with torch.no_grad():
    ...
    for roi in range(V):
        ...
        # Per-batch linear fit (for quick local visualization/debug)
        a_loc, b_loc, rc_lin_local, R2_loc, NRMSE_loc = lin_calibrate(rc, gt)

        # Train/Val calibration applied to this batch (if available)
        a_tv = b_tv = R2_tv = NRMSE_tv = np.nan
        if a_vec is not None and b_vec is not None:
            a_tv = float(a_vec[roi].item()); b_tv = float(b_vec[roi].item())
            rc_lin_tv = a_tv*rc + b_tv
            SSE = np.sum((gt - rc_lin_tv)**2); SST = np.sum((gt - gt.mean())**2) + 1e-8
            R2_tv = float(1.0 - SSE / SST)
            NRMSE_tv = float(np.sqrt(SSE/len(gt)) / (gt.std() + 1e-8))

        best_lag, r_best = lagged_corr(gt, rc, MAX_LAG_TR)

        # Keep original columns + appended *_tv
        diag_rows.append([
            roi, r0, float(gt.std()), float(rc.std()),
            a_loc, b_loc, R2_loc, NRMSE_loc,
            best_lag, r_best,
            a_tv, b_tv, R2_tv, NRMSE_tv
        ])
        rs.append((roi, r0))
    ...
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow([
            "roi","r0","std_gt","std_rec","slope","intercept","R2","NRMSE",
            "best_lag_TR","r_at_best_lag","slope_tv","intercept_tv","R2_tv","NRMSE_tv"
        ])
    ...
    # Optional Top-K plots (train/val-calibrated IF available; else per-batch)
    if MAKE_CALIB_PLOTS and len(top) > 0:
        ...
        for ax, (roi, r) in zip(axes, top):
            gt = x_true[:, roi]; rc = x_rec[:, roi]
            if a_vec is not None and b_vec is not None:
                a = float(a_vec[roi].item()); b = float(b_vec[roi].item())
                rc_lin = a*rc + b
                SSE = np.sum((gt - rc_lin)**2); SST = np.sum((gt - gt.mean())**2) + 1e-8
                R2  = 1.0 - SSE/SST
            else:
                a, b, rc_lin, R2, _ = lin_calibrate(rc, gt)  # fallback
