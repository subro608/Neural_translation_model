...
# -----------------------------
# Trainable translator (with CONDITIONAL embeddings)
# -----------------------------
class TranslatorModel(nn.Module):
    ...
    def forward(
        ...
        if condition_ids is not None:
            cond_vec = self.condition_embed(condition_ids)  # (B, D)
            eeg_adapted = eeg_adapted + cond_vec.unsqueeze(1)
            fmri_adapted = fmri_adapted + cond_vec.unsqueeze(1)
...
def train_loop(cfg: TrainConfig) -> None:
    ...
    if resume_path is not None:
        ...
        # (Scheduler state not restored; add if needed.)
