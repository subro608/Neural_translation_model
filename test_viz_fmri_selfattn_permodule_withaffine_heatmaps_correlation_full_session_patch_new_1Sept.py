...
def main():
    ...
    # Derive stride in TRs from STRIDE_SEC if defined; otherwise default to non-overlapping
    try:
        stride_T = max(1, int(STRIDE_SEC / float(TR)))
    except NameError:
        stride_T = chunk_T
    if stride_T > chunk_T:
        stride_T = chunk_T
    ...
    # Inference — center-only keep from each window; fill head/tail at boundaries
    with torch.inference_mode():
        ...
        while start < T:
            ...
            # Advance by stride
            if start + stride_T <= start:  # safety
                start = end
            else:
                start += stride_T
            if torch.cuda.is_available():
